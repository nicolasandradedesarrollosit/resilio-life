...
export const NavbarUser = ({ currentPageName }: NavbarUserProps) => {
  ...
  return (
    <>
      ...
      <nav
        className={`hidden md:flex flex-col items-center justify-start h-screen fixed top-0 left-0 transition-all duration-300 ease-out overflow-hidden ${
          isNavOpen
            ...
            {buttons.map((item, index) => (
              ...
              >
                ...
                {currentPageName === item.name && (
                  <span
                    aria-hidden
                    className="block h-1 w-8 bg-magenta-fuchsia-600 rounded-full shadow-sm"
                  />
                )}
              </Button>
            ))}

...
export default function ModalUpdateEvent({ id }: { id: string }) {
    ...
    useEffect(() => {
        if (isOpen && eventToUpdate) {
            // Pre-fill data
            setSelectedDate(parseDate(new Date(eventToUpdate.date).toISOString().split('T')[0]));
                ...
                {(onClose) => (
                    <>
                        <ModalHeader className="flex flex-col w-full items-center gap-4 sm:gap-5">
                            ...
                            />
                            <div>
                                <h2 className='text-white font-semibold text-lg sm:text-xl'>Modificar Evento</h2>
                                <p className='text-slate-400 text-xs sm:text-sm mt-1'>Actualiza los detalles del evento</p>
                            </div>
                        </ModalHeader>

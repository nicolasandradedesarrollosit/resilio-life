...
export default function LocationPicker({
    ...
    const selectSearchResult = (result: NominatimResult) => {
        ...
        if (mapRef.current) {
            ...
            const marker = L.marker([lat, lng]).addTo(mapRef.current)
            marker.bindPopup(`<b>${result.display_name.split(',')[0]}</b><br>${result.display_name}`).openPopup()
            markerRef.current = marker
        }
    ...
    return (
        <div className="w-full space-y-3">
            <div className="flex gap-2">
                    ...
                    {showResults && searchResults.length > 0 && (
                        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-48 overflow-y-auto">
                            {searchResults.map((result) => (
                                ...
                                >
                                    <span className="font-medium text-magenta-fuchsia-800">{result.display_name.split(',')[0]}</span>
                ...
                >
                    <Navigation2 size={18} className={isLocating ? "animate-pulse" : ""} />
                    <span className="hidden sm:inline">{isLocating ? "Localizando..." : "Mi ubicación"}</span>
                </button>
            </div>
            ...
            {selectedCoords && (
                <div className="text-sm text-gray-600 bg-gray-50 rounded-lg p-2">
                    <MapPin size={16} className="inline text-magenta-fuchsia-800" /> Lat: <span className="font-mono">{selectedCoords.lat.toFixed(6)}</span>,
                    <MapPin size={16} className="inline text-magenta-fuchsia-800" /> Lng: <span className="font-mono">{selectedCoords.lng.toFixed(6)}</span>
                </div>
            )}
